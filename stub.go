...
type Stub interface {
	...
	is(method string, req reflect.Type, res reflect.Type) bool
	resp() (any, error)
	schema() string
	use() Stub
	check(t *testing.T)
...
func (s stub[I, O]) is(method string, req reflect.Type, res reflect.Type) bool {
	return reflect.TypeOf(s.request) == req && reflect.TypeOf(s.response) == res && s.method == method
}
